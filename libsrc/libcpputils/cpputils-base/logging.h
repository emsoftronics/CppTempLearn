...
class LogdLogger {
 public:
  explicit LogdLogger(LogId default_log_id = cpputils::base::MAIN);
...
#define SEVERITY_LAMBDA(severity) ([&]() {    \
  using ::cpputils::base::VERBOSE;             \
  using ::cpputils::base::DEBUG;               \
  using ::cpputils::base::INFO;                \
  using ::cpputils::base::WARNING;             \
  using ::cpputils::base::ERROR;               \
  using ::cpputils::base::FATAL_WITHOUT_ABORT; \
  using ::cpputils::base::FATAL;               \
  return (severity); }())
...
#define ABORT_AFTER_LOG_EXPR_IF(c, x) (((c) && ::cpputils::base::LogAbortAfterFullExpr()) || (x))
// Note to the static analyzer that we always execute FATAL logs in practice.
#define MUST_LOG_MESSAGE(severity) (SEVERITY_LAMBDA(severity) == ::cpputils::base::FATAL)
#else
...
#define WOULD_LOG(severity) \
  (UNLIKELY((SEVERITY_LAMBDA(severity)) >= ::cpputils::base::GetMinimumLogSeverity()) || \
...
#define LOG_STREAM_TO(dest, severity)                                           \
  ::cpputils::base::LogMessage(__FILE__, __LINE__, ::cpputils::base::dest,        \
...
#define LOGGING_PREAMBLE(severity)                                                         \
  (WOULD_LOG(severity) &&                                                                  \
   ABORT_AFTER_LOG_EXPR_IF((SEVERITY_LAMBDA(severity)) == ::cpputils::base::FATAL, true) && \
   ::cpputils::base::ErrnoRestorer())
...
#define CHECK(x)                                                                 \
  LIKELY((x)) || ABORT_AFTER_LOG_FATAL_EXPR(false) ||                            \
      ::cpputils::base::LogMessage(__FILE__, __LINE__, ::cpputils::base::DEFAULT,  \
                                  ::cpputils::base::FATAL, _LOG_TAG_INTERNAL, -1) \
              .stream()                                                          \
          << "Check failed: " #x << " "
...
#define CHECK_OP(LHS, RHS, OP)                                                                 \
  for (auto _values = ::cpputils::base::MakeEagerEvaluator(LHS, RHS);                           \
...
#define DCHECK(x) \
  if (::cpputils::base::kEnableDChecks) CHECK(x)
#define DCHECK_EQ(x, y) \
  if (::cpputils::base::kEnableDChecks) CHECK_EQ(x, y)
#define DCHECK_NE(x, y) \
  if (::cpputils::base::kEnableDChecks) CHECK_NE(x, y)
#define DCHECK_LE(x, y) \
  if (::cpputils::base::kEnableDChecks) CHECK_LE(x, y)
#define DCHECK_LT(x, y) \
  if (::cpputils::base::kEnableDChecks) CHECK_LT(x, y)
#define DCHECK_GE(x, y) \
  if (::cpputils::base::kEnableDChecks) CHECK_GE(x, y)
#define DCHECK_GT(x, y) \
  if (::cpputils::base::kEnableDChecks) CHECK_GT(x, y)
#define DCHECK_STREQ(s1, s2) \
  if (::cpputils::base::kEnableDChecks) CHECK_STREQ(s1, s2)
#define DCHECK_STRNE(s1, s2) \
  if (::cpputils::base::kEnableDChecks) CHECK_STRNE(s1, s2)
#if defined(NDEBUG) && !defined(__clang_analyzer__)
