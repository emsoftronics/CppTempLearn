...
#include "cpputils-base/scopeguard.h"
...
TEST(scopeguard, normal) {
  ...
  {
    auto scopeguard = cpputils::base::make_scope_guard([&guarded_var] { guarded_var = false; });
  }
...
TEST(scopeguard, vector) {
  int guarded_var = 0;
  {
    std::vector<cpputils::base::ScopeGuard<std::function<void()>>> scopeguards;
    scopeguards.emplace_back(cpputils::base::make_scope_guard(
