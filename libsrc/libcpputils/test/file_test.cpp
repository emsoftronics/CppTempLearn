...
TEST(file, ReadFileToString_ENOENT) {
  std::string s("hello");
  errno = 0;
  ASSERT_FALSE(cpputils::base::ReadFileToString("/proc/does-not-exist", &s));
...
TEST(file, ReadFileToString_WriteStringToFile) {
  TemporaryFile tf;
  ASSERT_TRUE(tf.fd != -1);
  ASSERT_TRUE(cpputils::base::WriteStringToFile("abc", tf.path))
    << strerror(errno);
  std::string s;
  ASSERT_TRUE(cpputils::base::ReadFileToString(tf.path, &s))
    << strerror(errno);
  EXPECT_EQ("abc", s);
}
...
TEST(file, ReadFileToString_WriteStringToFile_symlink) {
  ...
  ASSERT_FALSE(cpputils::base::WriteStringToFile("foo", link.path, false));
  ASSERT_EQ(ELOOP, errno);
  ASSERT_TRUE(cpputils::base::WriteStringToFile("foo", link.path, true));

  std::string s;
  ASSERT_FALSE(cpputils::base::ReadFileToString(link.path, &s));
  ASSERT_EQ(ELOOP, errno);
  ASSERT_TRUE(cpputils::base::ReadFileToString(link.path, &s, true));
  ASSERT_EQ("foo", s);
}
#endif
...
TEST(file, WriteStringToFile2) {
  TemporaryFile tf;
  ASSERT_TRUE(tf.fd != -1);
  ASSERT_TRUE(cpputils::base::WriteStringToFile("abc", tf.path, 0660,
                                               getuid(), getgid()))
      << strerror(errno);
  struct stat sb;
...
TEST(file, WriteStringToFd) {
  TemporaryFile tf;
  ASSERT_TRUE(tf.fd != -1);
  ASSERT_TRUE(cpputils::base::WriteStringToFd("abc", tf.fd));

  ASSERT_EQ(0, lseek(tf.fd, 0, SEEK_SET)) << strerror(errno);

  std::string s;
  ASSERT_TRUE(cpputils::base::ReadFdToString(tf.fd, &s)) << strerror(errno);
  EXPECT_EQ("abc", s);
}
...
TEST(file, WriteFully) {
  TemporaryFile tf;
  ASSERT_TRUE(tf.fd != -1);
  ASSERT_TRUE(cpputils::base::WriteFully(tf.fd, "abc", 3));

  ASSERT_EQ(0, lseek(tf.fd, 0, SEEK_SET)) << strerror(errno);

  std::string s;
  s.resize(3);
  ASSERT_TRUE(cpputils::base::ReadFully(tf.fd, &s[0], s.size()))
    << strerror(errno);
  EXPECT_EQ("abc", s);
  ...
  s.resize(1024);
  ASSERT_FALSE(cpputils::base::ReadFully(tf.fd, &s[0], s.size()));
}
...
TEST(file, RemoveFileIfExists) {
  ...
  tf.fd = -1;
  std::string err;
  ASSERT_TRUE(cpputils::base::RemoveFileIfExists(tf.path, &err)) << err;
  ASSERT_TRUE(cpputils::base::RemoveFileIfExists(tf.path));
  TemporaryDir td;
  ASSERT_FALSE(cpputils::base::RemoveFileIfExists(td.path));
  ASSERT_FALSE(cpputils::base::RemoveFileIfExists(td.path, &err));
...
TEST(file, RemoveFileIfExists_ENOTDIR) {
  ...
  tf.fd = -1;
  std::string err{"xxx"};
  ASSERT_TRUE(cpputils::base::RemoveFileIfExists(std::string{tf.path} + "/abc", &err));
  ASSERT_EQ("xxx", err);
}
...
TEST(file, RemoveFileIfExists_EACCES) {
  ...
  ASSERT_FALSE(cpputils::base::RemoveFileIfExists(tf.path, &err));
  ASSERT_EQ("Permission denied", err);
  // Set dir's search permission again.
  ASSERT_TRUE(chmod(td.path, S_IRWXU) == 0);
  ASSERT_TRUE(cpputils::base::RemoveFileIfExists(tf.path, &err));
}
#endif
...
TEST(file, Readlink) {
  ...
  result = "wrong";
  ASSERT_TRUE(cpputils::base::Readlink(min_path, &result));
  ASSERT_EQ(min, result);

  result = "wrong";
  ASSERT_TRUE(cpputils::base::Readlink(max_path, &result));
...
#if !defined(_WIN32)
  TemporaryDir td;
  std::string basename = cpputils::base::Basename(td.path);
  std::string dir_name = cpputils::base::Dirname(td.path);
  std::string base_dir_name = cpputils::base::Basename(dir_name);

  {
    std::string path = dir_name + "/../" + base_dir_name + "/" + basename;
    std::string result;
    ASSERT_TRUE(cpputils::base::Realpath(path, &result));
...
TEST(file, GetExecutableDirectory) {
  std::string path = cpputils::base::GetExecutableDirectory();
  ASSERT_NE("", path);
  ASSERT_NE(cpputils::base::GetExecutablePath(), path);
  ASSERT_EQ('/', path[0]);
  ASSERT_NE('/', path[path.size() - 1]);
}

TEST(file, GetExecutablePath) {
  ASSERT_NE("", cpputils::base::GetExecutablePath());
}

TEST(file, Basename) {
  EXPECT_EQ("sh", cpputils::base::Basename("/system/bin/sh"));
  EXPECT_EQ("sh", cpputils::base::Basename("sh"));
  EXPECT_EQ("sh", cpputils::base::Basename("/system/bin/sh/"));
}

TEST(file, Dirname) {
  EXPECT_EQ("/system/bin", cpputils::base::Dirname("/system/bin/sh"));
  EXPECT_EQ(".", cpputils::base::Dirname("sh"));
  EXPECT_EQ("/system/bin", cpputils::base::Dirname("/system/bin/sh/"));
}
...
TEST(file, ReadFileToString_capacity) {
  ...
  ASSERT_TRUE(cpputils::base::WriteStringToFile(std::string(size, 'x'), tf.path));
  ASSERT_TRUE(cpputils::base::ReadFileToString(tf.path, &s));
...
TEST(file, ReadFileToString_capacity_0) {
  ...
  ASSERT_TRUE(cpputils::base::WriteStringToFile("", tf.path));
  ASSERT_TRUE(cpputils::base::ReadFileToString(tf.path, &s));
  EXPECT_EQ(0U, s.size());
