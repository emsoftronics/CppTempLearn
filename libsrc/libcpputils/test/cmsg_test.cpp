...
using cpputils::base::ReceiveFileDescriptors;
using cpputils::base::SendFileDescriptors;
using cpputils::base::unique_fd;
...
struct CmsgTest : ::testing::TestWithParam<bool> {
  ...
  void SetUp() override {
    ASSERT_TRUE(
        cpputils::base::Socketpair(Seqpacket() ? SOCK_SEQPACKET : SOCK_STREAM, &send, &recv));
