...
#include <cpputils-base/threads.h>
    ...
    static const char *getprogname()
    {
      ...
      if (first)
      {
        snprintf(progname, sizeof(progname), "%s",
                 cpputils::base::Basename(cpputils::base::GetExecutablePath()).c_str());
        first = false;
      }
...
#define PATH_MAX_STRING_SIZE 256
    ...
    /* recursive mkdir */
    static int CreateDirPath(const char *dir, const mode_t mode)
    {
    ...
    static int OpenKmsg()
    {
      ...
      {
        int fd;
        if (cpputils::base::ParseInt(val, &fd, 0))
        {
...
#if defined(__linux__)
    void KernelLogger(cpputils::base::LogId, cpputils::base::LogSeverity severity,
                      const char *tag, const char *, unsigned int, const char *msg)
    {
      // clang-format off
  static constexpr int kLogSeverityToKernelLogLevel[] = {
      [cpputils::base::VERBOSE] = 7,              // KERN_DEBUG (there is no verbose kernel log
                                                 //             level)
      [cpputils::base::DEBUG] = 7,                // KERN_DEBUG
      [cpputils::base::INFO] = 6,                 // KERN_INFO
      [cpputils::base::WARNING] = 4,              // KERN_WARNING
      [cpputils::base::ERROR] = 3,                // KERN_ERROR
      [cpputils::base::FATAL_WITHOUT_ABORT] = 2,  // KERN_CRIT
      [cpputils::base::FATAL] = 2,                // KERN_CRIT
  };
      // clang-format on
      static_assert(arraysize(kLogSeverityToKernelLogLevel) == cpputils::base::FATAL + 1,
    ...
    static void *GetLogFile()
    {
      ...
      if (!ret)
      {
        const char *fname = GetFileBasename(LOG_FILE);
        if (fname && *fname)
        {
          int dret = 0;
          ...
          if (tmp)
          {
            ...
            if (dname && (*dname))
            {
              ...
              if (!((len == 1) && ((*dname == '.') || (*dname == '/'))))
              {
                dret = CreateDirPath(tmp, 0777);
              }
            }
            free(tmp);
          }
                      ...
                      const char *message)
    {
      ...
      FILE *outfile = (FILE *)GetLogFile();
      outfile = (outfile) ? outfile : stderr;
      fprintf(outfile, "%s %c %s %5d %5" PRIu64 " %s:%u] %s\n", tag ? tag : "nullptr", severity_char,
                     ...
                     unsigned int /*line*/, const char *message)
    {
      ...
      if (severity >= WARNING)
      {
        ...
        fprintf((outfile) ? outfile : stderr, "%s: %s\n", GetFileBasename(getprogname()), message);
        fflush((outfile) ? outfile : stderr);
      }
      else
      {
