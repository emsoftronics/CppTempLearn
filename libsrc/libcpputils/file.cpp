...
#include "cpputils-base/logging.h" // and must be after windows.h for ERROR
#include "cpputils-base/macros.h"  // For TEMP_FAILURE_RETRY on Darwin.
#include "cpputils-base/unique_fd.h"
...
namespace cpputils
{
  namespace base
  {
    ...
    bool ReadFileToString(const std::string &path, std::string *content, bool follow_symlinks)
    {
      ...
      cpputils::base::unique_fd fd(TEMP_FAILURE_RETRY(open(path.c_str(), flags)));
      if (fd == -1)
      {
        return false;
      }
                           ...
                           bool follow_symlinks)
    {
      ...
      cpputils::base::unique_fd fd(TEMP_FAILURE_RETRY(open(path.c_str(), flags, mode)));
      if (fd == -1)
      {
        PLOG(ERROR) << "cpputils::WriteStringToFile open failed";
        return false;
      }
      ...
      if (fchmod(fd, mode) == -1)
      {
        PLOG(ERROR) << "cpputils::WriteStringToFile fchmod failed";
        return CleanUpAfterFailedWrite(path);
      }
      if (fchown(fd, owner, group) == -1)
      {
        PLOG(ERROR) << "cpputils::WriteStringToFile fchown failed";
        return CleanUpAfterFailedWrite(path);
      }
      if (!WriteStringToFd(content, fd))
      {
        PLOG(ERROR) << "cpputils::WriteStringToFile write failed";
        return CleanUpAfterFailedWrite(path);
      }
      return true;
    }
                           ...
                           bool follow_symlinks)
    {
      int flags = O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC | O_BINARY |
                  (follow_symlinks ? 0 : O_NOFOLLOW);
      cpputils::base::unique_fd fd(TEMP_FAILURE_RETRY(open(path.c_str(), flags, 0666)));
      if (fd == -1)
      {
        return false;
      }
    ...
    std::string GetExecutablePath()
    {
      ...
      std::string path;
      cpputils::base::Readlink("/proc/self/exe", &path);
      return path;
